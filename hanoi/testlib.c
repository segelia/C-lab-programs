...
#define MOVED_DISK_COLOUR MAGENTA
...
void draw_animated_block(int x, int y, int size)
{
	...
	gfx_filledRect(x - disk_width / 2, y + HEIGHT_OF_DISK, x + disk_width / 2, y, MOVED_DISK_COLOUR);
}
...
bool move_block(int gamestate[NO_OF_BLOCKS][NO_OF_PEGS], int source_peg, int destination_peg)
{
	...
	{
		if (gamestate[block][source_peg] != 0)
			break;
	}
	...
	{
		if (gamestate[block][destination_peg] == 0)
			break;
	}
	...
	{
		draw_view(gamestate);
		gfx_textout(gfx_screenWidth() / 4, gfx_screenHeight() / 6, "Block is moving.", WHITE);
		draw_animated_block(animated_x, animated_y, temporary_storage);
...
int main(int argc, char *argv[])
{
	...
	while (playing)
	{
		...
		if (!move_block(gamestate, source_peg, destination_peg))
		{
			...
			gfx_textout(gfx_screenWidth() / 2, gfx_screenHeight() / 2, "You lost.", TEXT_COLOUR);
			gfx_updateScreen();
			playing = false;
			break;
		}
		...
		if (is_win(gamestate))
		{
			...
			gfx_textout(gfx_screenWidth() / 2, gfx_screenHeight() / 2, "Congratulations.", TEXT_COLOUR);
			gfx_updateScreen();
			playing = false;
			break;
		}
